...
public:
	...
	Sprite(Surface* a_Surface, unsigned int a_NumFrames, float a_FrameInterval);
	~Sprite();
	// Methods
	void Draw( Surface* a_Target, int a_X, int a_Y, bool flipX = false );
	...
	void NextFrame(float deltaTime) { 
		if (deltaTime + m_FrameTime > m_FrameInterval) {
			m_FrameTime = 0;
			if (m_CurrentFrame == m_NumFrames - 1) m_CurrentFrame = 0;
			else m_CurrentFrame += 1;
		}
		else m_FrameTime += deltaTime;
	}
